 ...
 * @author Ruzaik Mh
 */
@WebServlet(name = "UpdateSupplyDetails", urlPatterns = {"/UpdateSupplyDetails"})
public class UpdateSupplyDetails extends HttpServlet {
     ...
     protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
       ...
       out.write(sp1.toString());
        ...
        try {
            ...
            String query = "UPDATE `jewellry`.`supply` SET `InvoiceNo` = ?,`Date` = ?,`Name` = ?,`Discount` = ?,`PaymentType` = ?,`Category` = ?,`Qty` = ?,`Weight` = ?,`UnitPrice` = ?,`Total` = ?,`Description` = ? where ID='"+sp1.getID()+"'";
            ...
            request.getRequestDispatcher("/editSupply.jsp").forward(request,response);
